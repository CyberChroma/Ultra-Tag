...
public class AIPathfindingInput : MonoBehaviour
{
    ...
    private CharacterStateTracker itCharacterTracker;
    ...
    void Start()
    {
        ...
        itCharacterTracker = FindFirstObjectByType<CharacterStateTracker>();
    ...
    /*void GetDestination()
    {
        if (CharacterStateTracker.hunterCharacters[0].transform != lastITCharacter)
        {
            ITCharacterChange();
        }
        else
        ...
        {
            if (currDestination == endDestination && !CharacterStateTracker.hunterCharacters.Contains(transform))
            {
        ...
        lastITCharacter = CharacterStateTracker.hunterCharacters[0].transform;
        lastITClosestPath = itClosestPath;
    }

    void ITCharacterChange()
    {
        itCharacterClosestPathfinding = CharacterStateTracker.hunterCharacters[0].GetComponentInChildren<CharacterClosestPathfinding>();
        itClosestPath = itCharacterClosestPathfinding.closestWaypoint;
        NewEndDestination();
    }
    ...
    {
        if (CharacterStateTracker.hunterCharacters.Contains(transform))
        {
            ...
            {
                if (allCharacterClosestPathfinding[i] != selfCharacterClosestPathfinding && !CharacterStateTracker.hunterCharacters.Contains(allCharacterClosestPathfinding[i].transform.parent))
                {
    ...
    void TurnInput()
    {
        ...
        if (CharacterStateTracker.hunterCharacters.Contains(transform) && (Mathf.Abs(transform.position.y - closestCharacterPos.y) < 5) && (Vector3.Distance(transform.position, closestCharacterPos) < Vector3.Distance(transform.position, currDestination.transform.position) || currDestination == endDestination))
        {
